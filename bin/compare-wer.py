...
def compare(wer_result1, wer_result2, name1, name2, template, css, corpus_without_top_words):
    ...
    for line1, line2 in zip(wer_result1, wer_result2):
        try:
            ...
            hyp2 = hyp2.replace('*', '.')
            # print(ref, hyp1, hyp2)
            # print(are_lemma_equal(ref,hyp1))
            # print(are_lemma_equal(ref,hyp2))
            ...
            if are_lemma_equal(hyp1, ref):
                ...
                hyp1_class = 'false'
                wrong_words_baseline.append(ref)
            ...
            if are_lemma_equal(hyp2, ref):
                ...
                hyp2_class = 'false'
                wrong_words_interpolated.append(ref)
            ...
            if in_(ref, corpus):
                ...
                if hyp1_class == 'false':
                    wrong_interesting_words_baseline.append(ref)
                if hyp2_class == 'false':
                    wrong_interesting_words_interpolated.append(ref)
    ...
    lemmatized_reference_words = map(get_lemmatized_form, reference_words)
    interesting_words = [w for w in lemmatized_reference_words if w in corpus]

    log('interesting_words: ', interesting_words);
    log('corpus: ', corpus);
    log('lemmatized_reference_words: ', lemmatized_reference_words);
    print_key_value('KW', interesting_words)
...
def main_():
    ...
    corpus = map(get_lemmatized_form, open(args[4]).read().split())
